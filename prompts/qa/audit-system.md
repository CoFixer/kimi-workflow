---
description: Comprehensive audit of .kimi configuration system (overlaps, drift, triggers, architecture, completeness)
...
### Step 2: Detect .kimi System Paths

Read `.kimi/stack-config.json` to determine enabled stacks.
...
# Always include base
$SKILL_RULES_PATHS += ".kimi/skills/skill-rules.json"        # root level
$AGENT_DIRS += ".kimi/base/agents/"

# Add per enabled stack
for stack in enabled_stacks:
    if exists ".kimi/${stack}/skills/skill-rules.json":
        $SKILL_RULES_PATHS += it
    if exists ".kimi/${stack}/agents/":
        $AGENT_DIRS += it
    if exists ".kimi/${stack}/guides/":
        $GUIDE_DIRS += it

# Check for standalone marketing module
if exists ".kimi/marketing/skills/skill-rules.json":
    $SKILL_RULES_PATHS += it

# Hook paths
$HOOK_DIRS += ".kimi/hooks/"
$HOOK_DIRS += ".kimi/base/hooks/"
for stack in enabled_stacks:
    if exists ".kimi/${stack}/hooks/":
        $HOOK_DIRS += it

$SETTINGS_PATH = ".kimi/settings.json"
$GITMODULES_PATH = ".kimi/.gitmodules"
$STACK_CONFIG_PATH = ".kimi/stack-config.json"
```
...
1. **Hook registration vs files**: Read `settings.json`. Extract every hook command path. For each path:
   - Replace `.kimi` with the actual project directory
...
- `/validate-claude-config` - Configuration structure validation (subset of this command's drift + architecture dimensions)
