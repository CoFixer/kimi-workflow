...
You are a full project setup assistant. This command creates a new project with .kimi config, boilerplate code, and .project documentation.
...
Options:
  1. Full Project Setup (Recommended)
     Description: "Create new project with boilerplate code, .kimi config, and documentation"
...
**Note:** Compress mode creates lightweight versions of AGENTS.md and README.md by removing verbose sections, reducing file sizes by ~40-50%.
...
Add the shared claude-workflow as `.kimi` submodule:

```bash
git submodule add https://github.com/$GITHUB_ORG/claude-workflow.git .kimi
...
|-----------|------------|--------|-------|
| NestJS | `https://github.com/$GITHUB_ORG/nestjs-starter-kit` | `backend/` | Backend framework |
| Django | `https://github.com/$GITHUB_ORG/django-starter-kit` | `backend/` | Backend framework |
| React Web | `https://github.com/$GITHUB_ORG/react-starter-kit` | `frontend/` | Web application |
| Dashboard | `https://github.com/$GITHUB_ORG/react-starter-kit` | `dashboard/` | Consolidated dashboard for admin/organizer/ops-manager roles |
| React Native | `https://github.com/$GITHUB_ORG/react-native-starter-kit` | `mobile/` | Mobile app |
...
if [ "$BACKEND" = "nestjs" ]; then
  git clone --branch main --single-branch https://github.com/$GITHUB_ORG/nestjs-starter-kit backend/
  ...
  git clone --branch main --single-branch https://github.com/$GITHUB_ORG/django-starter-kit backend/
  rm -f backend/Dockerfile* backend/docker-compose*.yml backend/.dockerignore
  rm -rf backend/.git backend/.gitmodules
...
if [[ " ${FRONTENDS[@]} " =~ " react " ]]; then
  git clone --branch main --single-branch https://github.com/$GITHUB_ORG/react-starter-kit frontend/
...
if [ "$CREATE_DASHBOARD" = true ]; then
  if [ "$DASHBOARD_STRATEGY" = "separate" ]; then
    ...
    for role in "${DETECTED_ROLES[@]}"; do
      ...
      git clone --branch main --single-branch https://github.com/$GITHUB_ORG/react-starter-kit "dashboard-${role}/"
    ...
    git clone --branch main --single-branch https://github.com/$GITHUB_ORG/react-starter-kit dashboard/
...
if [[ " ${FRONTENDS[@]} " =~ " react-native " ]]; then
  git clone --branch main --single-branch https://github.com/$GITHUB_ORG/react-native-starter-kit mobile/
...
cp -r .kimi/templates/project/* .project/
```
...
required_files=(".project/docs/PROJECT_KNOWLEDGE.md" ".project/docs/PROJECT_API.md" ".project/docs/PROJECT_DATABASE.md" "AGENTS.md" "README.md")
...
grep -q "^## Quick Stack Reference" "AGENTS.md" || { echo "✗ AGENTS.md missing required sections"; exit 1; }
...
**Integration Point**: Executes automatically during `/new-project` when HTML prototypes are detected.

### 7.9 Create AGENTS.md

Create consolidated context file in root for token-efficient Claude interactions:

```bash
# Copy template to root
cp .kimi/templates/AGENTS.template.md AGENTS.md

# Replace placeholders
sed -i '' "s/{PROJECT_NAME}/$PROJECT_NAME/g" AGENTS.md
sed -i '' "s/{BACKEND}/$BACKEND/g" AGENTS.md
sed -i '' "s/{FRONTENDS}/$FRONTENDS_STR/g" AGENTS.md
...
if [[ ! " ${FRONTENDS[@]} " =~ " react " ]]; then
  sed -i '' '/\[if react\]/,/\[endif\]/d' AGENTS.md
fi

# Remove [if react-native] sections if react-native not selected
if [[ ! " ${FRONTENDS[@]} " =~ " react-native " ]]; then
  sed -i '' '/\[if react-native\]/,/\[endif\]/d' AGENTS.md
fi

# Remove dashboard sections if not selected
if [ "$CREATE_DASHBOARD" != true ]; then
  sed -i '' '/\[if dashboard\]/,/\[endif\]/d' AGENTS.md
fi

if [[ ! " ${DASHBOARDS[@]} " =~ " analytics " ]]; then
  sed -i '' '/\[if analytics-dashboard\]/,/\[endif\]/d' AGENTS.md
fi

if [[ ! " ${DASHBOARDS[@]} " =~ " coach " ]]; then
  sed -i '' '/\[if coach-dashboard\]/,/\[endif\]/d' AGENTS.md
fi

# Clean up remaining markers
sed -i '' '/\[if .*\]/d' AGENTS.md
sed -i '' '/\[endif\]/d' AGENTS.md
...
if [ "$COMPRESS_MODE" = true ]; then
  ...
  sed -i '' '/^## Core BASH Tools/,/^## Essential Commands/d' AGENTS.md
  echo "✓ Created compressed AGENTS.md (~900 tokens)"
else
  echo "✓ Created AGENTS.md (~1500 tokens)"
fi
```
...
```bash
gh repo create $GITHUB_ORG/$PROJECT_NAME --private --description "$PROJECT_NAME - Full stack application"
```
...
```bash
gh repo edit $GITHUB_ORG/$PROJECT_NAME --default-branch dev
```
...
GitHub: https://github.com/$GITHUB_ORG/$PROJECT_NAME (private)
...
```bash
# Clean up local
rm -rf backend frontend frontend-dashboard mobile .kimi .project docker-compose.yml
