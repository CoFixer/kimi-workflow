...
Step 1   → Detect currently installed submodules under .kimi
...
| Directory | `pwd && basename "$(pwd)"` | If in project root with `.kimi/` subdirectory: `cd .kimi`. If neither `.kimi/` dir: `⚠️ Navigate to your project's .kimi directory first.` |
...
⚠️  WARNING: base is the core infrastructure submodule.
    ...
    It provides 14 symlinks used by the entire .kimi system:
    agents, commands, docs, guides, hooks, memory, monitoring,
    orchestration, presentations, react-native, scripts, skills,
    templates, brand

    Removing base will BREAK your entire .kimi configuration.
...
```
The following submodules will be permanently removed from your local .kimi setup:
...
| `Not in .kimi directory` | Wrong location | `cd .kimi/` |
