...
You are a project initialization assistant. Your task is to set up a new project with the modular `.kimi` submodule architecture.
...
gh repo create $GITHUB_ORG/$PROJECT_NAME-claude --public --description "Claude Code configuration for $PROJECT_NAME"

# Clone it locally
git clone https://github.com/$GITHUB_ORG/$PROJECT_NAME-claude.git /tmp/$PROJECT_NAME-claude
...
```bash
git submodule add https://github.com/$GITHUB_ORG/claude-base.git base
...
**For NestJS:**
```bash
git submodule add https://github.com/$GITHUB_ORG/claude-nestjs.git nestjs
git reset HEAD .gitmodules nestjs 2>/dev/null || true
```

**For Django:**
```bash
git submodule add https://github.com/$GITHUB_ORG/claude-django.git django
...
**For React Web:**
```bash
git submodule add https://github.com/$GITHUB_ORG/claude-react.git react
git reset HEAD .gitmodules react 2>/dev/null || true
```

**For React Native:**
```bash
git submodule add https://github.com/$GITHUB_ORG/claude-react-native.git react-native
...
git submodule add https://github.com/$GITHUB_ORG/$PROJECT_NAME-claude.git .kimi

# Initialize all nested submodules
git submodule update --init --recursive

# Commit
git add .gitmodules .kimi
...
✓ Created project-specific Claude config repo:
  https://github.com/$GITHUB_ORG/$PROJECT_NAME-claude
...
| Repo | URL |
|------|-----|
| claude-base | https://github.com/$GITHUB_ORG/claude-base |
| claude-nestjs | https://github.com/$GITHUB_ORG/claude-nestjs |
| claude-django | https://github.com/$GITHUB_ORG/claude-django |
| claude-react | https://github.com/$GITHUB_ORG/claude-react |
| claude-react-native | https://github.com/$GITHUB_ORG/claude-react-native |
