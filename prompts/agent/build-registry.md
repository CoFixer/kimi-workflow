...
You are a Claude Code registry builder. Your task is to scan the `.kimi` directory structure and generate a `kimi-registry.json` file that indexes all resources and their cross-references.
...
find .kimi -path "*commands/*/*.md" -type f 2>/dev/null | sort
```
...
{
  "source": "base",
  "file": ".kimi/prompts/agent/ralph.md",
...
find .kimi -path "*skills/*.md" -type f 2>/dev/null | grep -v README | sort

# Skill directories with SKILL.md
find .kimi -name "SKILL.md" -type f 2>/dev/null | sort

# skill-rules.json files
find .kimi -name "skill-rules.json" -type f 2>/dev/null | sort
```
...
find .kimi -path "*agents/*.md" -type f 2>/dev/null | grep -v README | sort
```
...
find .kimi -path "*guides/*.md" -type f 2>/dev/null | sort
find .kimi -path "*examples/*.md" -type f 2>/dev/null | sort
```
...
grep -r "design-qa" CLAUDE.md .kimi/*/README.md .kimi/prompts/*/*.md 2>/dev/null
```
...
Combine all discovered data into `kimi-registry.json`:
...
**Write the file:**
```bash
# The JSON will be written to .kimi/kimi-registry.json
```
...
Registry written to: .kimi/kimi-registry.json
