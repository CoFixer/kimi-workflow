...
**Purpose:** Scan existing `.kimi` framework submodules and add missing patterns to `.gitignore`.
...
- When upgrading from older `/setup-claude` versions
...
**Solutions:**
- Verify you're in the `.kimi` directory
- Check that `.gitmodules` file exists
- Ensure submodules were added via `/setup-claude` or `/migrate-submodules`
...
- Ensure you have write access to the `.kimi` directory
- Try running with appropriate permissions

## Related Commands

- `/setup-claude` - Interactive framework submodule setup (now includes automatic `.gitignore` updates)
...
- `/validate-claude-config` - Comprehensive Claude configuration validation
