...
Manage which technology stacks are loaded in your .kimi configuration.
...
# Validate .kimi directory
if [ ! -d "$CLAUDE_DIR" ]; then
    echo "❌ Error: .kimi directory not found at $CLAUDE_DIR"
    exit 1
fi
...
if [ -f "$GITMODULES" ]; then
    AVAILABLE_STACKS=($(grep "^\[submodule" "$GITMODULES" | sed 's/\[submodule "\(.*\)"\]/\1/' | grep -v "^\.kimi$"))
else
...
if [ "$CHANGES_MADE" = true ]; then
    ...
    if [ -d "$CLAUDE_DIR/prompts" ]; then
        prompt_count=$(find "$CLAUDE_DIR/prompts" -name "*.md" -type f 2>/dev/null | wc -l)
