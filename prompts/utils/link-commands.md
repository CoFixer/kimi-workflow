---
description: Verify .kimi/prompts/ directory structure and cross-reference with skills
argument-hint: no arguments
---

# /link-commands

Verifies that the `.kimi/prompts/` directory structure is complete and cross-references commands with registered skills.

> **Note:** In the current architecture, commands live directly in `.kimi/prompts/` subdirectories.
...
for dir in dev git project design qa agent utils; do
  if [ -d ".kimi/prompts/$dir" ]; then
    count=$(find ".kimi/prompts/$dir" -name "*.md" -type f | wc -l)
...
```bash
find .kimi/prompts -name "*.md" -type f | while read f; do
...
| Issue | Resolution |
|-------|------------|
| Missing prompts subdirectory | Check `.kimi/prompts/` structure |
...
- `/validate-claude-config` — Full configuration validation
