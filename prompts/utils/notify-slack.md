...
### Target Repositories
- `$GITHUB_ORG/claude-base`
- `$GITHUB_ORG/claude-nestjs`
- `$GITHUB_ORG/claude-react`
- `$GITHUB_ORG/claude-django`
...
# For each repo
gh api repos/$GITHUB_ORG/claude-base/commits -X GET -F since="$SINCE_DATE" --jq '.[] | {sha: .sha[0:7], author: .commit.author.name, date: .commit.author.date}'
gh api repos/$GITHUB_ORG/claude-nestjs/commits -X GET -F since="$SINCE_DATE" --jq '.[] | {sha: .sha[0:7], author: .commit.author.name, date: .commit.author.date}'
gh api repos/$GITHUB_ORG/claude-react/commits -X GET -F since="$SINCE_DATE" --jq '.[] | {sha: .sha[0:7], author: .commit.author.name, date: .commit.author.date}'
gh api repos/$GITHUB_ORG/claude-django/commits -X GET -F since="$SINCE_DATE" --jq '.[] | {sha: .sha[0:7], author: .commit.author.name, date: .commit.author.date}'
```
