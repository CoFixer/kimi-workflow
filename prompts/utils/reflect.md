...
| Flag | Description |
|------|-------------|
| `--on` | Enable automatic reflection at session end |
| `--off` | Disable automatic reflection |
| `--status` | Show reflection status and learning history |
| (no flag) | Run manual reflection analysis |

---

## Mode: Manual Reflection (default)

When called without flags, run the full reflection analysis below.
...
## Mode: `--on` — Enable Auto-Reflection

1. Create/update `.project/state/reflect-enabled.json`:
```json
{
  "enabled": true,
  "lastReflection": null,
...
```
2. If file exists, preserve `lastReflection` and `totalReflections`
3. Output: "Auto-reflection ENABLED"

---

## Mode: `--off` — Disable Auto-Reflection

1. Update `.project/state/reflect-enabled.json`:
```json
{
  "enabled": false,
  "lastReflection": "[preserved]",
  "totalReflections": [preserved]
}
```
2. Output: "Auto-reflection DISABLED"

---

## Mode: `--status` — Show Status

1. Read `.project/state/reflect-enabled.json` → show enabled/disabled
2. Count entries in `.project/memory/LEARNINGS.md`, `PREFERENCES.md`, `CORRECTIONS.md`
3. Show last 5 git commits with "reflect" in message
4. Output formatted status table

---

## Mode: Manual Reflection
