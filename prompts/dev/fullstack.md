...
#### Bootstrap .kimi in Target Project (Phase 1)
...
git submodule add -b dev https://github.com/$GITHUB_ORG/claude-workflow.git .kimi
git submodule update --init --recursive
```

This ensures the target project becomes self-sufficient — after the first run, it has its own `.kimi` and can run `/fullstack` natively.
...
if [ ! -d ".kimi/backend" ]; then
  ...
  echo ""
  echo "  cd .kimi"
  echo "  git submodule add https://github.com/$GITHUB_ORG/claude-$BACKEND.git $BACKEND"
  echo "  git submodule update --init --recursive"
  echo "  git add -A && git commit -m 'feat: Add claude-$BACKEND submodule'"
  echo "  git push"
  echo "  cd .."
  echo "  git add .kimi && git commit -m 'chore: Update .kimi submodule'"
  echo ""
  exit 1
fi
```
...
for frontend in "${FRONTENDS[@]}"; do
  if [ ! -d ".kimi/frontend" ]; then
    ...
    echo ""
    echo "  cd .kimi"
    echo "  git submodule add https://github.com/$GITHUB_ORG/claude-$frontend.git $frontend"
    echo "  git submodule update --init --recursive"
...
|-----------|---------------|
| nestjs | `https://github.com/$GITHUB_ORG/claude-nestjs.git` |
| django | `https://github.com/$GITHUB_ORG/claude-django.git` |
| react | `https://github.com/$GITHUB_ORG/claude-react.git` |
| react-native | `https://github.com/$GITHUB_ORG/claude-react-native.git` |
...
See `.kimi/base/docs/PHASE_RESULT_SCHEMA.md` for the full schema, size limits, and context checkpoint protocol.
...
Load the template from `.kimi/base/templates/PIPELINE_STATUS.template.md` and replace `{PROJECT_NAME}` and `{DATE}` placeholders.

**Key sections:**
- `## Progress` — phase status table
- `## Execution Log` — history of phase runs
- `## Configuration` — project metadata and tech stack
- `## Agent Results` — compact PHASE_RESULT from each phase
- `## Change Tracking` — submodule hashes, skill checksums, doc mtimes
...
1. Report error: "Skill file not found: {resolved_skill_path}. Check .kimi/backend/guides/ or .kimi/frontend/guides/ for the correct guide."
...
**Phase skill guides** (in `.kimi/backend/guides/` and `.kimi/frontend/guides/`) are comprehensive implementations:
...
Skill to follow: .kimi/backend/guides/workflow-convert-prd-to-knowledge.md
...
Project: {PROJECT}
Status file: .project/status/{PROJECT}/PIPELINE_STATUS.md
Skill to follow: .kimi/backend/guides/workflow-design-database.md
...
- .kimi/backend/guides/BEST-PRACTICES.md
...
Project: {PROJECT}
Status file: .project/status/{PROJECT}/PIPELINE_STATUS.md
Skill to follow: .kimi/frontend/guides/browser-testing.md
...
Project: {PROJECT}
Status file: .project/status/{PROJECT}/PIPELINE_STATUS.md
Skill to follow: .kimi/frontend/guides/common-patterns.md
