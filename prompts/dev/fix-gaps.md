...
5. NEVER run `git commit` — leave that to the user via `/commit-all`
...
**Continuation**: Re-running `/fix-gaps` (no args) resumes auto-cascade from the first tier that still has PENDING gaps. Any `IN PROGRESS` gaps from an interrupted run are treated as PENDING and included in scope.
...
> **STOP** — "No gap analysis report found. Run `/gap-finder` first."
...
| Situation | Action |
|-----------|--------|
| No report found | **STOP**: "Run `/gap-finder` first." |
