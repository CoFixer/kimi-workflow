...
3. Push `.kimi` itself **without nested submodule pointer changes**, create PR to `dev`
4. Sync parent repo with dev (conflict prevention)
5. Push the parent repo **without `.kimi` pointer changes**, create PR to `dev`
...
**Note:** Submodules and the parent repo are pushed independently. `.kimi` is pull-only for nested submodules. Use `/commit` for main project only (no submodules at all).
...
8. **`.kimi` excludes nested submodule changes** - Never include nested submodule pointer updates (nestjs, react, etc.) in `.kimi` commit. `.kimi` is pull-only for nested submodules.
...
- **`.kimi` is pull-only for nested submodules** - Never push nested submodule pointer changes from `.kimi`
...
### 2.2 Push `.kimi` Submodule (WITHOUT Nested Submodule Pointers)

After all nested submodules under `.kimi` (e.g., `nestjs`, `react`) are pushed, handle `.kimi` itself.
**CRITICAL:** `.kimi` must NOT include nested submodule pointer changes. It is "pull-only" for nested submodules.
...
if [ -z "$CLAUDE_BRANCH" ] || [ "$CLAUDE_BRANCH" = "main" ] || [ "$CLAUDE_BRANCH" = "dev" ]; then
  echo "⚠️ Submodule '.kimi' on invalid branch: $CLAUDE_BRANCH"
  # Use AskUserQuestion to ask user for branch name
  # Then: git checkout -b <user-branch> && git push -u origin <user-branch>
  # Update CLAUDE_BRANCH to the new branch name
fi

# Ensure dev exists in .kimi repo
if ! git show-ref --verify --quiet refs/heads/dev; then
  ORIG="$CLAUDE_BRANCH"
  git checkout main && git checkout -b dev && git push -u origin dev
  git checkout "$ORIG"
...
# Skip if only submodule pointer changes existed (no .kimi-own files changed)
STAGED_FILES=$(git diff --cached --name-only)
if [ -z "$STAGED_FILES" ]; then
  echo "ℹ️ No .kimi-specific changes to commit (only nested submodule pointers). Skipping .kimi push."
  cd ..
  # Continue to parent repo step
else
  # Commit .kimi's own files only (no nested submodule pointers)
  git commit -m "$(cat <<'EOF'
<type>: <description of .kimi changes>
...
EOF
  ...
  # Push to .kimi's own current branch
  git push origin "$CLAUDE_BRANCH"

  # Create PR targeting dev in .kimi repo (REQUIRED)
  gh pr create --base dev --head "$CLAUDE_BRANCH" --title "<type>: <description>" --body "$(cat <<'EOF'
## Summary
<1-3 bullet points describing .kimi changes>

## Note
Nested submodule pointers (nestjs, react) are excluded. `.kimi` is pull-only for nested submodules.
...
**STOP if PR creation fails for `.kimi`.**
...
- ✓ `.kimi` committed WITHOUT nested submodule pointer changes (pull-only)
- ✓ `.kimi` PR was created targeting `dev` (if it had own-file changes)
- ✓ Parent repo committed WITHOUT `.kimi` pointer changes
...
.kimi PR (no nested submodule pointers):
<PR URL> (or "Skipped - no .kimi-specific changes")
  - Branch: <branch>
  - Commit: <hash> - <commit message>

Parent Repo PR (no .kimi pointer changes):
<PR URL>
...
- ❌ Include nested submodule pointer changes in `.kimi` commit (pull-only)
...
- **`.kimi` is pull-only for nested submodules** - Never include nested submodule pointer changes (nestjs, react, etc.) in `.kimi` commit
