...
### 2.1 Pull All Nested Submodules in .kimi
...
### 2.2 Pull .kimi Submodule

After nested submodules are pulled, pull `.kimi` itself:
...
if [ -z "$CLAUDE_BRANCH" ]; then
  echo "⚠️ .kimi is in detached HEAD, skipping pull"
else
  echo "Pulling dev into .kimi (branch: $CLAUDE_BRANCH)"
  git pull origin dev
fi
...
If `.kimi` shows as modified (new commits), inform the user:
...
| Prompts directory missing | Check .kimi/prompts/ structure |
